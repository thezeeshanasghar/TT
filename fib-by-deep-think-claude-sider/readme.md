# MT5 Fibonacci + Support/Resistance Scalping Bot (Python)

A deployable, live-trading MetaTrader 5 (*MT5*) Python bot that scalps using **Fibonacci retracements** plus programmatic **Support/Resistance (S/R)** confluence. It enforces strict guardrails—1% max risk per trade, a daily loss cap, configurable maximum effective leverage, and a limit on open positions. By default it runs on 1-minute charts (*M1*) for fast execution. A **dry-run** toggle allows forward testing without sending real orders.

---

## Table of Contents
1. [Overview](#overview)  
2. [Key Features & Guardrails](#key-features--guardrails)  
3. [Configuration Parameters](#configuration-parameters)  
4. [How It Works Mechanically](#how-it-works-mechanically)  
5. [Key Tunable Parameters](#key-tunable-parameters)  
6. [How to Run It](#how-to-run-it)  
7. [Why This Structure Works for Live Scalping](#why-this-structure-works-for-live-scalping)  
8. [Compliance & Edge-Case Notes](#compliance--edge-case-notes)  
...
- Anchors Fibonacci levels from the most recent confirmed impulse leg, defined by objective swing pivots.  
- Builds Support/Resistance levels from recent micro-pivots, clustering nearby prices to reduce noise.  
- Signals only when the current price is within a tight tolerance of both a fib retracement and an S/R level (confluence).  
- Buys pullbacks during up-impulses and sells pullbacks during down-impulses, placing stops beyond the impulse origin and take-profits at a configurable risk:reward ratio.  
- Sizes positions so that per-trade risk ≤ 1% of equity and effective leverage ≤ configured maximum.  
- Enforces **hard guardrails**: daily loss cap, spread/volatility filters, maximum open positions, and optional NFA/FIFO no-hedging rules.  

---

## Key Features & Guardrails

- **Deterministic Fib Anchors**: Removes discretion by using swing-pivot rules.  
- **S/R Confluence**: Narrows entries to high-probability liquidity pockets on sub-5-minute charts.  
- **Risk-Based Sizing**: Position size = (equity × risk%) / (stop pips × pip value).  
- **Guardrails**:  
  - **Daily loss cap** (e.g., −3% realized PnL stops new trades).  
  - **Max open positions** across all symbols.  
  - **Spread** and **ATR** filters to avoid illiquid or low-volatility markets.  
  - **NFA/FIFO behavior**: prevents hedging if enabled.  

---

## Configuration Parameters

All parameters are in the `CONFIG` dictionary at the top of the script. You can adjust them before saving the file.

### Trading
| Parameter           | Default     | Description                                 |
|---------------------|-------------|---------------------------------------------|
| symbol              | "EURUSD"    | Trading symbol                              |
| timeframe           | `TIMEFRAME_M1` | Chart timeframe (M1 by default)            |
| lookback_bars       | 600         | Number of bars to fetch each loop           |
| poll_interval_sec   | 0.5         | Sleep time between loops (seconds)          |

### Fib / SNR Mechanics
| Parameter               | Default | Description                                                                                |
|-------------------------|---------|--------------------------------------------------------------------------------------------|
| swing_left_right        | 5       | Fractal swing pivot width (bars on each side)                                              |
| snr_pivot_width         | 3       | Pivot width for micro S/R levels                                                           |
| snr_lookback            | 300     | Bars to scan for S/R pivot levels                                                          |
| confluence_tolerance_pips | 0.5   | Price tolerance (pips) for matching fib & S/R                                              |
| min_impulse_pips        | 5       | Minimum size of the last impulse leg to anchor fibs                                        |
| atr_period              | 14      | Period for Average True Range (*ATR*) volatility check                                     |
| min_atr_pips            | 0.4     | Skip extremely low-volatility markets                                                      |

*Background Note:* ATR (*Average True Range*) is a technical measure of market volatility.

### Entries & Exits
| Parameter             | Default  | Description                                                   |
|-----------------------|----------|---------------------------------------------------------------|
| rr_ratio              | 1.2      | Risk:reward ratio (take-profit = rr_ratio × risk distance)    |
| stop_buffer_pips      | 1.0      | Additional buffer beyond swing for stop placement (pips)      |
| entry_mode            | "market" | `"market"` or `"limit"` entry                                  |
| limit_improve_pips    | 0.0      | Price improvement toward level when using limit orders        |

### Risk & Compliance
| Parameter                 | Default | Description                                                 |
|---------------------------|---------|-------------------------------------------------------------|
| risk_per_trade_pct        | 1.0     | Percent of equity risked per trade                          |
| daily_loss_cap_pct        | 3.0     | Stop new trades after this percent loss realized for the day|
| max_effective_leverage    | 5.0     | Maximum notional/equity ratio                               |
| max_open_positions        | 3       | Max concurrent open positions across account                |
| max_spread_pips           | 0.6     | Skip if spread (pips) exceeds this                           |
| nfa_fifo_no_hedging       | True    | Enforce no-hedging & FIFO closes (for NFA compliance)       |
| account_is_hedging        | False   | Set True if broker/account supports hedging accounts       |

### Execution & State
| Parameter           | Default    | Description                                                           |
|---------------------|------------|-----------------------------------------------------------------------|
| slippage_points     | 10         | Allowed deviation in _points_ for `order_send`                        |
| retry_on_requote    | 2          | Number of retries on requote/price-changed                            |
| dry_run             | False      | If True, logs actions without sending orders                          |
| daily_state_file    | "daily_state.json" | File to persist daily PnL start and realized values         |
...
1. **Swing Detection**  
   - Uses fractal pivots to identify the last impulse leg (high-low or low-high).  
   - Discards legs smaller than `min_impulse_pips`.  

2. **Fibonacci Levels**  
   - Computes 23.6%, 38.2%, 50.0%, 61.8%, 78.6% retracements from the impulse leg.  

3. **Support/Resistance Map**  
   - Scans recent micro-pivots and clusters close levels to form S/R.  

4. **Confluence Signal**  
   - Finds price levels where a fib retracement and an S/R level coincide within tolerance.  
   - Prioritizes deeper retracements (50%, 61.8%, 78.6%).  

5. **Directional Entry**  
   - For an up-impulse: buys pullback near the confluence level.  
   - For a down-impulse: sells pullback near the confluence level.  
   - Stop is placed beyond the impulse origin plus buffer; TP is set by `rr_ratio`.  

6. **Position Sizing**  
   - Risk per trade ≤ `risk_per_trade_pct` of equity.  
   - Capped by `max_effective_leverage`.  
   - Rounded to broker’s lot step.  

7. **Guardrails & Filters**  
   - Daily loss cap (realized PnL since local midnight).  
   - Maximum open positions.  
   - Spread and ATR filters.  
   - Optional NFA/FIFO no-hedging: prevents opposite-direction entries if positions exist.  
...
| timeframe                   | Chart period (M1, M2, M5, etc.)         |
| risk_per_trade_pct          | Size of each trade (keep ≤ 1% as recommended) |
| daily_loss_cap_pct          | Risk budget per day (e.g., 3.0)         |
| max_effective_leverage      | Leverage ceiling on notional exposure   |
| max_open_positions          | Position count limit                    |
| confluence_tolerance_pips   | Tighten for fewer entries               |
| rr_ratio                    | Exit profile and profit target          |
| stop_buffer_pips            | Stop placement conservatism             |
| nfa_fifo_no_hedging         | Set for NFA compliance                  |
| account_is_hedging          | Match your broker’s account model       |
...
1. Install requirements:  
   ...
   ```  
2. Open MetaTrader 5 terminal and log in to your live or demo account.  
3. Save the script as `mt5_fib_snr_scalper.py` on the same machine.  
4. (Optional) Set `CONFIG["dry_run"] = True` for forward testing without real orders.  
5. Execute:  
   ```bash
   python mt5_fib_snr_scalper.py
   ```  

---

## Why This Structure Works for Live Scalping

- **Deterministic Anchors**: Removes manual bias in fib placement.  
- **Confluence Entries**: Targets liquidity pockets where mean-reversions cluster.  
- **Risk-Based Sizing**: Adapts to varying volatility and spread regimes.  
- **Hard Guardrails**: Prevents runaway losses under adverse conditions.  

---

## Compliance & Edge-Case Notes

- **NFA/FIFO Behavior**: If `nfa_fifo_no_hedging=True`, the bot blocks opposite-direction entries while positions exist.  
- **Netting vs. Hedging Accounts**: For netting accounts, set `account_is_hedging=False`.  
- **Midnight Definition**: Uses local machine midnight for daily PnL. Adjust if your broker’s server day differs.  

---

## Go-Live Checklist

- Verify symbol properties in MT5 (contract size, volume step/min/max, tick size/value).  
- Confirm pip-value math by placing a small test trade.  
- Start with `dry_run=True` to validate entries on M1 charts.  
- Set `daily_loss_cap_pct` and `max_effective_leverage` per your risk policy.  
- Monitor for order rejections; adjust `slippage_points` and `entry_mode` as needed.  

---

## Potential Enhancements

- Session/time filters (e.g., London/New York overlaps).  
- Dynamic tolerance scaling with ATR.  
- Partial profit-taking at 1R and break-even stop on the remainder.  
- Kill-switch if spread spikes beyond recent percentiles.  
