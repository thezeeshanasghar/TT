...
# --- CALCULATE TRAILING STOP (EXACT PINE SCRIPT LOGIC) ---
def calculate_trailing_stop(df, key_value=1.0, atr_period=10, use_heikin_ashi=False):
    ...
    # Initialize trailing stop (Pine Script logic)
    trailing_stop = [df.iloc[0]['src'] - df.iloc[0]['nLoss']]  # First value: src - nLoss
    ...
    for i in range(1, len(df)):
        ...
        # Pine Script trailing stop logic:
...
# --- GET TRADING SIGNAL (EXACT PINE SCRIPT LOGIC) ---
def get_trading_signal():
    ...
    if len(df) < 3:  # Need at least 3 bars for proper calculation
        return "no_signal"
    ...
    prev2_src = df.iloc[-3]['src']
    prev2_ts = df.iloc[-3]['trailing_stop']
    ...
    # buy = src > trailingStop and ta.crossover(ema1, trailingStop)
    # sell = src < trailingStop and ta.crossover(trailingStop, ema1)
    
    # Crossover function (ta.crossover in Pine Script)
    # crossover(a, b) => a[1] <= b[1] and a > b
...
def main_trading_loop():
    ...
    while True:
        try:
            ...
            if signal != "no_signal" and signal != last_signal:
                signal_count += 1
                print(f"\n🎯 SIGNAL #{signal_count}: {signal.upper()} detected!")
                print(f"🔄 Auto-reversing position...")
                
                # Place the trade
                if place_trade(signal):
                    last_signal = signal
                    print(f"✅ Successfully reversed to {signal.upper()} position")
                else:
